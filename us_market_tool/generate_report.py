# -*- coding: utf-8 -*-
"""
US Market Decision Support Tool - HTML Report Generator
매일 GitHub Actions에서 실행 → docs/index.html 자동 생성
"""
...
def safe_download_yf(tickers, period="2y", auto_adjust=True):
    ...
    if df.empty:
        raise ValueError("Yahoo Finance download returned empty DataFrame.")
    if isinstance(df.columns, pd.MultiIndex):
        ...
        for t in tickers:
            ...
            if "Close" in sub.columns:
                out_list.append(sub["Close"].rename(t))
        if not out_list:
            raise ValueError("No Close columns found.")
...
def score_vix(vix):
    ...
    return score, {"vix": curr, "vix_5d_chg": chg5, "vix_ma20": ma20,
                   "vix_ratio20": ratio20, "vix_high10": high10, "vix_dd_from_10d_high": dd}
...
def score_tactical(price, asset):
    ...
    return score, {"dist20": dist20, "ret10": ret10, "ma20": ma20, "ma5": ma5,
                   "cond_cross_5dma": cross5, "cond_2_of_3_up": two_of_3, "cond_rebound_3pct": rebound}
...
def score_breadth_proxy(proxy_ratio, asset):
    ...
    if len(s) < 60:
        ...
        return score, {"proxy_ratio": np.nan, "proxy_ma50": np.nan,
                       "proxy_roc20": np.nan, "bucket": "neutral_fallback", "approx_breadth": 0.50}
    curr = s.iloc[-1]
    ...
    return score, {"proxy_ratio": curr, "proxy_ma50": ma50,
                   "proxy_roc20": roc20, "bucket": bucket, "approx_breadth": ab}
...
def build_results():
    ...
    for asset, price, bf, rates_fn, trend_fn in [
        ...
        results[asset] = AssetResult(
            asset=asset, total_score=total,
            raw_signal=raw, final_signal=final,
...
def score_bar(score, max_score=100):
    ...
    <div style="background:#1a1a2e;border-radius:4px;height:8px;width:100%;overflow:hidden;">
      <div style="height:100%;width:{pct:.1f}%;background:{color};border-radius:4px;transition:width 1s ease;"></div>
    </div>"""
...
def bool_badge(v):
    if v:
        return "<span style='color:#00d084;font-size:11px;'>✓</span>"
    return "<span style='color:#444;font-size:11px;'>✗</span>"


def generate_html(results: Dict[str, AssetResult]) -> str:
    ...
    for asset, r in results.items():
        ...
        bm = r.module_meta["breadth_proxy"]
        ...
        if r.guardrail_reasons:
            ...
            <div class="guardrail-box">
              <div class="section-label">⚠ 가드레일 발동</div>
              <ul style="margin:6px 0 0 16px;padding:0;color:#f0b429;font-size:12px;">{items}</ul>
            </div>"""

        max_scores = {"SPY": {"trend": 35, "vix": 25, "tactical": 15, "breadth_proxy": 15, "rates": 10},
                      "QQQ": {"trend": 30, "vix": 25, "tactical": 15, "breadth_proxy": 10, "rates": 20}}
        ms = max_scores.get(asset, {k: 25 for k in sc})

        module_rows = ""
        labels = {"trend": "추세", "vix": "VIX", "tactical": "전술", "breadth_proxy": "Breadth", "rates": "금리"}
        for mod, label in labels.items():
            ...
            <div class="mod-row">
              ...
              <span class="mod-score">{s}<span style="color:#444">/{mx}</span></span>
              {score_bar(s, mx)}
            </div>"""
        ...
        <div class="asset-card" style="border-top:3px solid {col};">
          <div class="card-header">
            <div>
              ...
              <div style="color:#555;font-size:12px;margin-top:2px;">{r.module_meta['proxy_info']['breadth_ticker']} breadth proxy</div>
            </div>
            <div style="text-align:right;">
              <div class="signal-badge" style="background:{bg};color:{col};border:1px solid {col}33;">{r.final_signal}</div>
              {raw_eq}
            </div>
          </div>

          <div class="score-display">
            <span style="color:{col};font-size:42px;font-weight:700;letter-spacing:-2px;">{r.total_score}</span>
            <span style="color:#333;font-size:20px;">/100</span>
          </div>
          ...
          <div class="details-grid">
            <div class="detail-group">
              <div class="section-label">추세</div>
              ...
              <div class="detail-row"><span>20일 수익률</span><span>{fmt(tm['ret20'],'pct')}</span></div>
            </div>
            <div class="detail-group">
              <div class="section-label">VIX</div>
              ...
              <div class="detail-row"><span>10일고점 대비</span><span>{fmt(vm['vix_dd_from_10d_high'],'pct')}</span></div>
            </div>
            <div class="detail-group">
              <div class="section-label">전술</div>
              ...
              <div class="detail-row"><span>5일저점 +3%</span><span>{bool_badge(xm['cond_rebound_3pct'])}</span></div>
            </div>
            <div class="detail-group">
              <div class="section-label">Breadth / 금리</div>
              ...
              <div class="detail-row"><span>20일 변화</span><span>{fmt(rm['delta20_bp'],'bp')}</span></div>
            </div>
          </div>
          {guardrail_html}
        </div>"""
...
<style>
  *, *::before, *::after {{ box-sizing: border-box; margin: 0; padding: 0; }}
  ...
  .asset-card:hover {{ border-color: #2a2a4a; }}
...
<div class="footer">
  ...
  <p style="margin-top:8px;color:#1a1a2e;">본 도구는 참고용이며 투자 권유가 아닙니다.</p>
</div>
...
if __name__ == "__main__":
    ...
    print(f"✓ docs/index.html 생성 완료 ({datetime.now(KST).strftime('%Y-%m-%d %H:%M KST')})")
    for asset, r in results.items():
